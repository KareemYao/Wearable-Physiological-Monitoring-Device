...
Send_AT_Test(UART_HandleTypeDef* AT_Channel,uint8_t* r_buffer){
    ...
    if((status = Send_AT_Command(AT_Channel,r_buffer,(uint8_t*)msg,sizeof(msg),WAIT_TIME_MS)) != HAL_OK)
        return status;
...
Soft_Reset_NearLink(UART_HandleTypeDef* AT_Channel,uint8_t* r_buffer){
     ...
     * 
     */ 
    status = Send_AT_Command_With_Ready_Detect(AT_Channel,r_buffer,(uint8_t*)msg,sizeof(msg));
